<?= view('admin/components/page-toolbar', [
    'title'   => 'Témoignages',
    'actions' => '<a href="' . site_url('admin/testimonials/create') . '" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg me-1"></i> Ajouter</a>',
]) ?>
<div class="admin-card"><div class="table-responsive"><table class="table admin-table table-hover mb-0">
<thead><tr>
<th>ID</th><th>Auteur (FR)</th><th>Note</th><th>Actif</th><th></th>
</tr></thead><tbody>
...
<a href="<?= site_url('admin/testimonials/' . $row['id'] . '/edit') ?>" class="btn btn-sm btn-outline-primary">Modifier</a>
<form action="<?= site_url('admin/testimonials/' . $row['id'] . '/delete') ?>" method="post" class="d-inline" onsubmit="return confirm('Supprimer ?')">
